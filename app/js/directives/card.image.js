app.directive('cardImage', [function() {
  return {
    ...
    template: '<div style="background: url({{ url }}) center center / cover no-repeat; min-height: 200px; min-width: 300px" layout layout-align="space-between center">' +
                '<a style="text-align: center" href="javascript:;" ng-click="go(\'backward\')" ng-hide="disabled" flex="10">' +
                  '<md-icon style="color: #fff">chevron_left</md-icon>' +
                '</a>' +
                '<h3 style="color: #fff" class="md-title" ng-hide="disabled">{{ index + 1 }} of {{ images.length }}</h3>' +
                '<a style="text-align: center" href="javascript:;" ng-click="go(\'forward\')" ng-hide="disabled" flex="10">' +
                  '<md-icon style="color: #fff">chevron_right</md-icon>' +
                '</a>' +
              '</div>',
    link: function(scope, element, attrs) {
      scope.images = ['img/card.png', 'img/card01.jpg', 'img/card02.jpg', 'img/card03.jpg', 'img/card04.jpg'];
